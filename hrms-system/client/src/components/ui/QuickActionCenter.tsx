...
export default function QuickActionCenter() {
  ...
  const actions = [
    { label: 'Register Candidate', icon: UserPlus, href: '/candidate-entry', target: '_blank', color: 'bg-[#4A1726]' },
    { label: 'Section Allocation', icon: Calendar, href: '/section-allocation', color: 'bg-indigo-600' },
    { label: 'Broadcast Notification', icon: Send, href: '/broadcast-center', color: 'bg-[#C6A15B]' },
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end">
          ...
          {actions.map((act, idx) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-3 px-3.5 py-2 rounded-xl bg-white border border-[#EAE4DC] shadow-xl hover:shadow-2xl transition-all duration-150 group text-left"
              >
                <span className="text-xs font-bold text-[#4A1726] whitespace-nowrap group-hover:text-[#C6A15B]">
                  {act.label}
                </span>
      ...
      <button
        ...
        className={`
          w-13 h-13 rounded-2xl bg-[#4A1726] text-white flex items-center justify-center shadow-2xl border border-white/20 transition-all duration-200 hover:scale-105 active:scale-95
          ${open ? 'rotate-45 bg-rose-600' : 'bg-[#4A1726]'}
        `}
