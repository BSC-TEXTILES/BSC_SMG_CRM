    ...
    <div className="min-h-screen bg-[#F8F5F1] flex">
      ...
      <div className="flex-1 lg:pl-64 flex flex-col min-w-0">
        ...
        <main className="p-4 lg:p-6 space-y-6 flex-1 overflow-y-auto">
          ...
          <div className="card-glass p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-black text-[#4A1726] tracking-tight flex items-center gap-2">
                <Megaphone className="w-6 h-6 text-[#C6A15B]" />
              ...
              {[
                ...
                <button
                  ...
                  className={`
                    ...
                    ${activeTab === t.key ? 'bg-[#4A1726] text-white shadow-md' : 'bg-white border border-[#EAE4DC] text-[#4A1726] hover:bg-[#F8F5F1]'}
                  `}
                >
                  {t.label}
                </button>
            ...
            <div className="space-y-6">
              ...
              <div className="card-glass p-6 space-y-4">
                <div className="flex items-center justify-between border-b border-[#EAE4DC] pb-3">
                  <h3 className="font-extrabold text-[#4A1726] text-base">Active Broadcast Announcements</h3>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-[#EAE4DC] text-[#777777] font-extrabold uppercase text-[10px] tracking-wider">
                    ...
                    <tbody className="divide-y divide-[#EAE4DC]">
                      {broadcasts.map((b, idx) => (
                        <tr key={b.id} className="hover:bg-[#F8F5F1] transition-colors">
                          ...
                          <td className="py-3 px-3">
                            <div className="font-extrabold text-[#4A1726]">{b.title}</div>
                            <div className="text-[10.5px] text-[#777777]">{b.subject || b.message.slice(0, 45)}</div>
                          </td>
                          <td className="py-3 px-3 font-semibold text-[#4A1726]">{b.category}</td>
                          <td className="py-3 px-3 font-bold text-[#C6A15B]">{b.targetRole || 'Everyone'}</td>
                          <td className="py-3 px-3 font-extrabold text-emerald-700">{b.status || 'Sent'}</td>
                          <td className="py-3 px-3 font-mono font-bold text-[#4A1726]">{b.acknowledgedBy?.length || 0}</td>
          ...
          {activeTab === 'create' && (
            <div className="card-glass p-6 max-w-4xl mx-auto space-y-6">
              <div className="border-b border-[#EAE4DC] pb-3 flex items-center gap-2">
                <Send className="w-5 h-5 text-[#C6A15B]" />
                <div>
                  <h3 className="font-extrabold text-[#4A1726] text-base">Create Enterprise Broadcast Notice</h3>
              ...
              <div className="space-y-4 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Broadcast Title *</label>
                    <input
                      type="text"
                      value={title}
                  ...
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Subject / Header</label>
                    <input
                      type="text"
                      value={subject}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Priority Level</label>
                    <select
                      value={priority}
                  ...
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Broadcast Category</label>
                    <select
                      value={category}
                ...
                <div>
                  <label className="block font-bold text-[#4A1726] mb-2">Target Audience / Recipient Groups *</label>
                    ...
                    {RECIPIENT_ROLES.map(r => (
                      <button
                        ...
                        className={`
                          ...
                          ${targetRoles.includes(r) ? 'bg-[#4A1726] text-white border-[#4A1726]' : 'bg-[#F8F5F1] border-[#EAE4DC] text-[#4A1726] hover:bg-white'}
                        `}
                      >
                        {r}
                      </button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Schedule Dispatch Date &amp; Time (Optional)</label>
                    <input
                      type="datetime-local"
                      value={scheduledAt}
                  ...
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Expiry Date (Optional)</label>
                    <input
                      type="date"
                      value={expiryDate}
                ...
                {/* Messaging Content */}
                <div>
                  <label className="block font-bold text-[#4A1726] mb-1">Broadcast Message Body * (Text Only)</label>
                  <textarea
                    rows={5}
                    value={message}
                ...
                {/* Options Checkboxes */}
                <div className="p-4 rounded-2xl bg-[#F8F5F1] border border-[#EAE4DC] space-y-2">
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-[#4A1726]">
                    <input
                      type="checkbox"
                ...
                {/* Actions */}
                <div className="flex items-center justify-end gap-3 pt-3 border-t border-[#EAE4DC]">
                  <button
                    type="button"
                    onClick={() => handleDispatch(true)}
                    className="px-4 py-2.5 rounded-xl border border-[#EAE4DC] font-bold text-xs bg-white hover:bg-[#F8F5F1]"
                  >
                    Save Draft
                  </button>
                  <button
          ...
          {activeTab === 'history' && (
            <div className="card-glass p-6 space-y-4">
              <h3 className="font-extrabold text-[#4A1726] text-base border-b border-[#EAE4DC] pb-3">Complete Broadcast Audit &amp; History Log</h3>
              <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
                {broadcasts.map((b) => (
                  <div key={b.id} className="p-4 rounded-2xl border border-[#EAE4DC] bg-[#F8F5F1] space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="font-extrabold text-sm text-[#4A1726]">{b.title}</span>
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-[#4A1726]/10 text-[#4A1726]">
                    ...
                    <div className="flex items-center justify-between text-[10px] text-[#888888] pt-1 border-t border-[#EAE4DC]/50 font-semibold">
                      <span>Sender: <strong className="text-[#4A1726]">{b.senderName || 'HR Desk'}</strong></span>
