...
const MODULE_BARS = ['bg-[#4A1726]', 'bg-[#C6A15B]', 'bg-emerald-600', 'bg-purple-600', 'bg-rose-600', 'bg-teal-600'];
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-black text-[#4A1726] flex items-center gap-2">
                <BarChart3 className="w-7 h-7 text-[#C6A15B]" /> Daily MCheck Management Dashboard
              </h1>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                onClick={loadReport}
                className="p-2.5 rounded-xl bg-white border border-gray-200 hover:bg-[#4A1726] hover:text-white hover:border-[#4A1726] transition-all shadow-sm cursor-pointer"
              ...
              <div className="flex items-center gap-1 rounded-xl bg-gray-100 p-1">
                {(['date', 'range'] as const).map(m => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${filterMode === m ? 'bg-[#4A1726] text-white shadow' : 'text-gray-500 hover:text-gray-800'}`}
                  >
            ...
            <>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
                  {[
                    { label: 'Total Checkpoints', value: kpi?.total ?? 0, bg: 'bg-[#4A1726]', icon: Target },
                    ...
                    { label: 'Overall Completion', value: `${kpi?.completionPct ?? 0}%`, bg: 'bg-[#C6A15B]', icon: BarChart3 },
                  ].map((item, i) => {
                    ...
                    return (
                      <div key={i} className="bg-white border border-gray-100 p-4 rounded-2xl shadow-xs flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="text-xl font-black text-[#4A1726] leading-tight">{item.value}</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    {moduleStats.length > 0 ? moduleStats.map((m, i) => (
                      <div
                        ...
                        className="p-3 rounded-xl border border-gray-100 hover:border-[#C6A15B] hover:bg-amber-50/20 transition-all cursor-pointer group"
                      >
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <span className="font-extrabold text-sm text-[#4A1726] group-hover:text-[#C6A15B] transition-colors truncate">
                              {m.module_name}
                            </span>
                            <ArrowRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-[#C6A15B] group-hover:translate-x-1 transition-all" />
                          </div>
                          <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                            <span className="text-xs font-bold text-gray-500">{m.done} / {m.total}</span>
                            <span className={`text-sm font-black ${m.completion_pct === 100 ? 'text-emerald-600' : m.completion_pct >= 50 ? 'text-[#4A1726]' : 'text-red-500'}`}>
                              {m.completion_pct}%
                            </span>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-xs">
                  <h2 className="text-sm font-black text-gray-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-[#C6A15B]" /> DAILY TREND
                  </h2>
                    ...
                    {trendData.slice(-10).map((t, i, arr) => {
                      ...
                      return (
                        <div key={t.date} className="flex items-center gap-2">
                          ...
                          <div className="text-[11px] font-black text-[#4A1726] w-10 text-right">{t.completion_pct}%</div>
                        ...
                        <div key={i} className="p-4 hover:bg-gray-50 transition-colors">
                          <div className="flex items-start justify-between gap-4 flex-wrap">
                            <div className="flex-1 min-w-0 space-y-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="font-extrabold text-[#4A1726] text-sm">{item.checkpoint_title}</span>
                              ...
                              <div className="text-xs text-gray-600 flex items-center gap-4 flex-wrap">
                                <span>Responsible: <strong className="text-[#4A1726]">{item.responsible_person || item.responsible_department || 'Assigned Staff'}</strong></span>
                    ...
                    >
                      <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="font-bold text-[#4A1726] text-sm mt-0.5">{cp.checkpoint_title}</div>
                      ...
                      {expandedCheckpoint === i && (
                        <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs animate-fade-in bg-[#FAF9F7] p-3 rounded-xl">
                          <div><span className="font-bold text-gray-500">Compliance Status:</span> <strong className="text-[#4A1726]">{cp.compliance_status || '—'}</strong></div>
                          <div><span className="font-bold text-gray-500">Accuracy:</span> <strong className="text-[#4A1726]">{cp.accuracy || '—'}</strong></div>
                          <div><span className="font-bold text-gray-500">Updated By:</span> <strong className="text-[#4A1726]">{cp.updated_by || cp.submitted_by || '—'}</strong></div>
                          <div><span className="font-bold text-gray-500">Updated Date/Time:</span> <strong className="text-[#4A1726]">{cp.response_updated_at ? new Date(cp.response_updated_at).toLocaleString('en-IN') : cp.response_date || '—'}</strong></div>
                          ...
                          {cp.photo_url && (
                            <div className="sm:col-span-2 flex items-center gap-2">
                              ...
                              <a href={cp.photo_url} target="_blank" rel="noreferrer" className="text-xs text-[#C6A15B] font-bold underline flex items-center gap-1">
