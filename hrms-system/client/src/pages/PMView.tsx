                ...
                <tbody className="divide-y divide-gray-100">
                  {diverts.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/80 transition-colors">
                      <td className="p-4 font-black text-[#4A1726]">#{item.refNo || item.id.slice(0, 6)}</td>
                      <td className="p-4 font-bold text-[#4A1726]">{item.productWanted}</td>
        ...
        {selectedDivert && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <div className="card-glass p-6 max-w-md w-full animate-scale-in">
              <h3 className="text-lg font-black text-[#4A1726] mb-4">Update Sourcing Status</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Product</label>
                  <div className="font-extrabold text-sm text-[#4A1726]">{selectedDivert.productWanted}</div>
                </div>
