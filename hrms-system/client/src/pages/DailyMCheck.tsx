...
const MODULE_COLORS = [
  { bar: 'bg-[#4A1726]', ring: 'border-[#4A1726]', light: 'bg-blue-50' },
  { bar: 'bg-[#C6A15B]', ring: 'border-[#C6A15B]', light: 'bg-amber-50' },
...
function Toast({ msg, type }: { msg: string; type: 'success' | 'error' | 'info' }) {
  const bg = type === 'success' ? 'bg-emerald-600' : type === 'error' ? 'bg-red-600' : 'bg-[#4A1726]';
  return (
...
export default function DailyMCheck() {
  ...
  const KpiCard = ({ label, value, color, icon: Icon }: any) => (
    <div className="card-glass p-4 flex items-center gap-3 min-w-0 bg-white border border-gray-100 shadow-sm rounded-2xl">
      ...
      <div className="min-w-0">
        <div className="text-2xl font-black text-[#4A1726] leading-tight">{value}</div>
  ...
  return (
    <div className="min-h-screen bg-[#F8F5F1] flex">
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pl-64">
        ...
        <div className="flex-1 overflow-y-auto px-4 lg:px-6 py-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-black text-[#4A1726] flex items-center gap-2">
                <ClipboardList className="w-7 h-7 text-[#C6A15B]" />
                Daily MCheck
              </h1>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[#4A1726] text-[#4A1726] text-sm font-bold bg-white shadow-sm hover:bg-[#4A1726] hover:text-white transition-all"
                >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[['Today', 0], ['Yesterday', 1]].map(([label, offset]) => (
                        <button
                          ...
                          className="text-xs font-bold px-3 py-2 rounded-lg bg-[#4A1726]/5 text-[#4A1726] hover:bg-[#4A1726] hover:text-white transition-all"
                        >
                          {label as string}
                        </button>
                      ))}
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 hover:text-[#4A1726] hover:border-[#4A1726] text-xs font-bold transition-all shadow-sm"
              ...
              <button
                ...
                className="p-2.5 rounded-xl bg-white border border-gray-200 hover:bg-[#4A1726] hover:text-white hover:border-[#4A1726] transition-all shadow-sm"
                title="Refresh"
              >
          ...
          {!selectedModule ? (
            <>
              {/* Date Banner */}
              <div className="bg-[#4A1726] rounded-2xl p-5 text-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-md border border-white/10">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-[#C6A15B] mb-1">BSC SMG CRM — DAILY MANAGEMENT CHECKLIST</div>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <div className="text-4xl font-black text-[#C6A15B]">{dashLoading ? '—' : `${kpi?.completionPct || 0}%`}</div>
                  ...
                  <div className="w-14 h-14 rounded-full border-4 border-white/20 flex items-center justify-center relative">
                    <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 56 56">
                      ...
                      <circle
                        cx="28" cy="28" r="24" fill="none" stroke="#C6A15B" strokeWidth="5"
              ...
              {dashLoading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
                  <KpiCard label="Total" value={kpi?.total ?? 0} color="bg-[#4A1726]" icon={Target} />
                  ...
                  <KpiCard label="Completion" value={`${kpi?.completionPct ?? 0}%`} color="bg-[#C6A15B]" icon={Zap} />
                </div>
              )}
                  ...
                  {moduleStats.map((mod: any, idx: number) => {
                    ...
                    return (
                      <button
                        ...
                        className="bg-white border border-gray-200 rounded-2xl text-left p-5 group transition-all cursor-pointer hover:shadow-lg hover:border-[#C6A15B] shadow-xs"
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1 min-w-0">
                            <div className="font-black text-[#4A1726] text-base truncate group-hover:text-[#C6A15B] transition-colors">
                              {mod.module_name}
                            </div>
                          ...
                          <div className="ml-2 flex flex-col items-end">
                            <span className={`text-2xl font-black ${pct === 100 ? 'text-emerald-600' : pct >= 50 ? 'text-[#4A1726]' : 'text-amber-600'}`}>
                              {pct}%
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 pt-2 text-[11px] text-[#C6A15B] font-bold flex items-center justify-between">
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              <button
                ...
                className="flex items-center gap-2 text-sm font-bold text-[#4A1726] hover:text-[#C6A15B] transition-colors"
              >
              ...
              {moduleLoading ? (
                <div className="bg-white border border-gray-200 p-12 flex items-center justify-center rounded-2xl shadow-sm">
                  <div className="flex items-center gap-3 text-gray-500">
                    <div className="w-5 h-5 border-2 border-[#4A1726] border-t-transparent rounded-full animate-spin" />
                ...
                <>
                  ...
                  <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div>
                        <div className="text-2xl font-black text-[#4A1726]">{moduleData.module.module_name}</div>
                        <div className="text-xs text-[#C6A15B] font-bold mt-0.5">{dashData?.dateDisplay || selectedDate}</div>
                        <div className="text-sm text-gray-600 font-medium mt-1">
                          Module Progress: <span className="font-bold text-[#4A1726]">{moduleData.stats.done} of {moduleData.stats.total} submitted</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#4A1726] text-white text-xs font-bold hover:bg-[#350E1A] transition-all shadow-sm disabled:opacity-60"
                        >
                          {submitAllLoading ? (
                    ...
                    {moduleData.checkpoints.map((cp: any, idx: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="font-extrabold text-[#4A1726] text-sm truncate">
                                  {cp.checkpoint_title}
                                </div>
                          ...
                          {isExpanded && (
                            <div className="px-5 pb-6 border-t border-gray-100 pt-5 space-y-5 bg-[#FCFBF9]/60 animate-fade-in">
                              ...
                              <div className="bg-white border border-gray-200 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                                <div>
                                  ...
                                  <strong className="text-[#4A1726] text-sm">{cp.checkpoint_title}</strong>
                                </div>
                                <div>
                              ...
                              <div>
                                ...
                                {resp.photo_url ? (
                                  ...
                                  <label className="flex items-center gap-2 cursor-pointer w-fit px-4 py-2.5 rounded-xl border-2 border-dashed border-gray-300 hover:border-[#C6A15B] hover:bg-amber-50/50 transition-all text-xs text-gray-600 font-bold bg-white">
                              ...
                              <div className="flex items-center gap-3 pt-2 border-t border-gray-200">
                                ...
                                <button
                                  ...
                                  disabled={saving[cp.id]}
                                  className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#4A1726] text-white text-xs font-bold hover:bg-[#350E1A] transition-all disabled:opacity-60 shadow-md"
                                >
                                  {saving[cp.id] ? (
                                    <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                                  ) : (
      ...
      {showAdminModal && (
        <div className="fixed inset-0 bg-[#4A1726]/70 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-fade-in">
            {/* Modal Header */}
            <div className="bg-[#4A1726] px-6 py-4 text-white flex items-center justify-between">
              <div>
                <h3 className="font-black text-lg flex items-center gap-2">
                  <Settings className="w-5 h-5 text-[#C6A15B]" />
                    ...
                    return (
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <h4 className="font-black text-sm text-[#4A1726]">{currentMod.module_name} Checkpoints</h4>
                        ...
                        {(isAddingCheckpoint || editingCheckpoint) && (
                          <form onSubmit={handleSaveCheckpointAdmin} className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-3">
                            <div className="text-xs font-black text-[#4A1726] uppercase">
                          ...
                          {currentMod.checkpoints?.map((cp: any, i: number) => (
                            <div key={cp.id} className="p-3 flex items-center justify-between gap-3 hover:bg-gray-50 text-xs">
                              <div className="flex items-center gap-3 min-w-0 flex-1">
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="font-extrabold text-[#4A1726] truncate">{cp.checkpoint_title}</div>
                              ...
                              <div className="flex items-center gap-1.5 flex-shrink-0">
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-[#4A1726]/10 hover:bg-[#4A1726] hover:text-white transition-all text-[#4A1726]"
