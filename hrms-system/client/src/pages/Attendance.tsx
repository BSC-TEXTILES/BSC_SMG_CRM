  ...
  return (
    <DashboardLayout title="Attendance & Shift Roster Desk" subtitle="Daily Staff Attendance Tracking & Floor Shift Allocations">
      <div className="space-y-6">
        ...
        <div className="card-glass p-5 lg:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border border-[#EAE4DC]/80 bg-white/70 backdrop-blur-xl shadow-md rounded-2xl">
          <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#4A1726] text-[#C6A15B] flex items-center justify-center shadow-sm shrink-0">
              ...
              <div>
                ...
                <input
                  ...
                  className="px-3 py-1.5 rounded-xl border border-[#EAE4DC] bg-white font-extrabold text-xs text-[#4A1726] outline-none shadow-xs focus:ring-2 focus:ring-[#C6A15B]/40 transition-all"
                />
              </div>
            </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-extrabold transition-all ${
                    departmentFilter === dept
                      ? 'bg-[#4A1726] text-[#C6A15B] shadow-xs'
                      : 'bg-white/80 border border-[#EAE4DC] text-[#555555] hover:bg-white'
                  }`}
                >
                  {dept}
                </button>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-64">
              <input
                ...
                className="w-full text-xs font-semibold pl-9 pr-4 py-2 rounded-xl border border-[#EAE4DC] bg-white text-[#4A1726] focus:outline-none focus:border-[#C6A15B] focus:ring-2 focus:ring-[#C6A15B]/20 transition-all shadow-2xs"
              />
        ...
        <div className="card-glass overflow-hidden border border-[#EAE4DC]/80 shadow-lg rounded-2xl bg-white/80 backdrop-blur-xl">
          <div className="p-5 border-b border-[#EAE4DC] flex items-center justify-between bg-white/60">
            <div className="flex items-center gap-2">
              <UserCheck className="w-5 h-5 text-[#C6A15B]" />
              <h3 className="font-extrabold text-sm text-[#4A1726] uppercase tracking-wider">
                Daily Staff Attendance Register
              </h3>
            </div>
            <span className="badge b-sel font-extrabold text-xs py-1.5 px-3 rounded-full bg-[#4A1726]/5 border border-[#4A1726]/10 text-[#4A1726]">
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs font-semibold border-collapse">
                <thead className="bg-[#4A1726] text-white uppercase text-[10px] tracking-wider">
                  <tr>
                ...
                <tbody className="divide-y divide-[#EAE4DC]/60">
                  {filteredEmployees.map((emp) => (
                    <tr key={emp.id || emp.appNo} className="hover:bg-[#4A1726]/5 transition-colors font-medium">
                      <td className="p-4 font-mono font-black text-[#4A1726]">{emp.employeeCode || emp.empNo || emp.appNo || `EMP-${emp.id}`}</td>
                      ...
                      <td className="p-4 text-[#C6A15B] font-extrabold">{emp.desig || emp.designation || 'Staff'}</td>
