...
export default function EmployeesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F5F1] flex">
        ...
        <main className="p-4 lg:p-6 space-y-6 flex-1 overflow-y-auto">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl border border-[#EAE4DC] bg-white text-xs font-bold text-[#4A1726] hover:bg-[#F8F5F1] flex items-center gap-1.5 shadow-xs"
                >
                  <Download className="w-3.5 h-3.5" /> Sample
                </button>
                <label className="px-3 py-1.5 rounded-xl bg-[#4A1726] text-white text-xs font-bold cursor-pointer hover:bg-[#350E1A] flex items-center gap-1.5 shadow-xs">
              ...
              <select
                ...
                className="px-3 py-1.5 rounded-xl border border-[#EAE4DC] bg-[#F8F5F1] text-xs font-semibold text-[#4A1726]"
              >
              ...
              <select
                ...
                className="px-3 py-1.5 rounded-xl border border-[#EAE4DC] bg-[#F8F5F1] text-xs font-bold text-[#4A1726]"
              >
          ...
          <div className="card-glass p-5 space-y-4">
            <div className="flex items-center justify-between border-b border-[#EAE4DC] pb-3">
              <h3 className="font-extrabold text-[#4A1726] text-sm tracking-tight">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-[#EAE4DC] text-[10.5px] font-black uppercase text-[#777777] bg-[#F8F5F1]/60">
                    ...
                    <tr key={emp.appNo || idx} onClick={() => setDrawerEmp(emp)} className="hover:bg-black/5 cursor-pointer transition-colors font-medium">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3 group text-left">
                          ...
                          <div>
                            <span className="font-extrabold text-[#4A1726] group-hover:underline block">{formatName(emp.name)}</span>
                            {emp.email && <span className="text-[10px] text-[#888888] font-semibold truncate max-w-[150px] block">{emp.email}</span>}
                          </div>
                        </div>
                      </td>
                      <td className="py-3.5 px-4 text-[#4A1726] font-extrabold">{emp.desig || emp.designation || 'Staff'}</td>
                      <td className="py-3.5 px-4 text-[#555555] font-semibold">{emp.department || '—'}</td>
                      <td className="py-3.5 px-4 text-[#C6A15B] font-extrabold">{emp.section || 'Unassigned'}</td>
      ...
      {editModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-[#4A1726]/60 backdrop-blur-md transition-all animate-fade-in">
          <div className="relative w-full max-w-3xl max-h-[90vh] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-[#C6A15B]/40">
            {/* Modal Header */}
            <div className="bg-[#4A1726] text-white p-4 sm:p-5 flex items-center justify-between border-b border-[#C6A15B]/30">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#C6A15B] text-white font-black text-lg flex items-center justify-center shadow-md">
                  <Edit3 className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-extrabold text-white text-base sm:text-lg">Edit Employee Details — {editModal.emp?.name}</h3>
                  <div className="text-xs text-[#C6A15B] font-mono mt-0.5 font-bold">
            ...
            <div className="flex items-center gap-2 p-3 bg-[#F8F5F1] border-b border-[#EAE4DC] overflow-x-auto text-xs font-bold">
              {[
                ...
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl whitespace-nowrap transition-all flex items-center gap-1.5 text-xs font-extrabold ${
                    editTab === tab.id
                      ...
                      : 'text-[#555555] hover:bg-white hover:text-[#4A1726]'
                  }`}
                >
                  <span>{tab.label}</span>
                </button>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs bg-[#F8F5F1]">
              {/* TAB 1: BASIC INFO */}
              {editTab === 'basic' && (
                <div className="space-y-4 bg-white p-5 rounded-2xl border border-[#EAE4DC] shadow-xs">
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Full Employee Name *</label>
                    <input type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="input-modern font-extrabold text-[#4A1726]" required />
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Phone Number *</label>
                    ...
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Email Address</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Gender</label>
                    ...
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Date of Birth (DOB)</label>
                      <input type="date" value={editForm.dob} onChange={(e) => setEditForm({ ...editForm, dob: e.target.value })} className="input-modern" />
                    </div>
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Blood Group</label>
                  ...
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Aadhaar Card Number</label>
              ...
              {editTab === 'workplace' && (
                <div className="space-y-4 bg-white p-5 rounded-2xl border border-[#EAE4DC] shadow-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Designation Role</label>
                      <input type="text" value={editForm.desig} onChange={(e) => setEditForm({ ...editForm, desig: e.target.value })} className="input-modern font-extrabold text-[#4A1726]" />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Assigned Floor Section</label>
                      <input type="text" placeholder="e.g. Ethnic Wear, Silk, Cash Counter" value={editForm.section} onChange={(e) => setEditForm({ ...editForm, section: e.target.value })} className="input-modern font-extrabold text-[#C6A15B]" />
                    </div>
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Employment Status</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Branch Location</label>
                      <input type="text" value={editForm.branch} onChange={(e) => setEditForm({ ...editForm, branch: e.target.value })} className="input-modern" />
                    </div>
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Reporting Manager</label>
              ...
              {editTab === 'compensation' && (
                <div className="space-y-4 bg-white p-5 rounded-2xl border border-[#EAE4DC] shadow-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Base Monthly Salary (₹)</label>
                      <input type="text" placeholder="e.g. 20000" value={editForm.salary} onChange={(e) => setEditForm({ ...editForm, salary: e.target.value })} className="input-modern font-mono font-bold text-[#4A1726]" />
                    </div>
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Monthly Incentive / Bonus (₹)</label>
                  ...
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Date of Joining (DOJ)</label>
              ...
              {editTab === 'experience' && (
                <div className="space-y-4 bg-white p-5 rounded-2xl border border-[#EAE4DC] shadow-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Total Work Experience</label>
                    ...
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Prior / Retail Experience</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Previous Company</label>
                    ...
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Previous Designation</label>
                    ...
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Previous Salary</label>
                  ...
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">Highest Qualification</label>
              ...
              {editTab === 'personal' && (
                <div className="space-y-4 bg-white p-5 rounded-2xl border border-[#EAE4DC] shadow-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Father's Details</label>
                    ...
                    <div>
                      <label className="block font-bold text-[#4A1726] mb-1">Mother's Details</label>
                  ...
                  <div>
                    <label className="block font-bold text-[#4A1726] mb-1">HR &amp; Executive Remarks</label>
                    <textarea
                      rows={2}
            ...
            {/* Modal Sticky Footer */}
            <div className="flex items-center justify-between p-4 bg-[#F8F5F1] border-t border-[#EAE4DC]">
              <div className="text-[11px] text-[#777777] font-semibold">
                Changes persist directly to MySQL Database.
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => setEditModal({ open: false, emp: null })} className="px-4 py-2 rounded-xl border border-[#EAE4DC] bg-white font-extrabold text-xs text-[#555555]">
                  Cancel
                </button>
