  ...
  const HistoryCard = ({ entry, prev }: { entry: any; prev: any }) => {
    ...
    return (
      <div
        ...
        className={`card-glass rounded-2xl p-4 cursor-pointer transition-all border-2 ${isSelected ? 'border-[#C6A15B] shadow-lg' : 'border-transparent hover:border-gray-200 hover:shadow-md'}`}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <div className="font-black text-[#4A1726] text-sm">{entry.date_display}</div>
            <div className="text-[10px] text-gray-400 mt-0.5">{entry.total_checkpoints} checkpoints</div>
          </div>
          <div className="flex flex-col items-end">
            <div className={`text-xl font-black ${pct === 100 ? 'text-emerald-600' : pct >= 75 ? 'text-[#4A1726]' : pct >= 50 ? 'text-amber-600' : 'text-red-500'}`}>{pct}%</div>
            {diff !== null && (
        ...
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden mb-3">
          <div className={`h-full rounded-full transition-all duration-500 ${pct === 100 ? 'bg-emerald-500' : pct >= 75 ? 'bg-[#4A1726]' : pct >= 50 ? 'bg-amber-500' : 'bg-red-400'}`}
            style={{ width: `${pct}%` }} />
        </div>
        ...
        <div className="flex items-center gap-2 mt-3 pt-2 border-t border-gray-100">
          ...
          <span className="ml-auto text-[10px] text-[#C6A15B] font-bold flex items-center gap-0.5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-black text-[#4A1726] flex items-center gap-2">
                <History className="w-7 h-7 text-[#C6A15B]" /> MCheck History
              </h1>
              <p className="text-sm text-gray-500 mt-0.5">Historical daily management checklist records</p>
            </div>
            <button onClick={loadHistory} className="p-2.5 rounded-xl bg-white border border-gray-200 hover:bg-[#4A1726] hover:text-white hover:border-[#4A1726] transition-all shadow-sm w-fit">
              ...
              {!selectedDate ? (
                ...
                <div className="card-glass rounded-2xl p-10 flex items-center justify-center">
                  <div className="flex items-center gap-3 text-gray-400">
                    <div className="spinner border-[#4A1726]" />
                ...
                <div className="space-y-4 animate-fade-in">
                  {/* Day header */}
                  <div className="bg-[#4A1726] rounded-2xl p-5 text-white">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-[10px] font-black text-[#C6A15B] uppercase tracking-widest mb-1">DAILY REVIEW</div>
                      ...
                      <div className="text-right">
                        <div className="text-3xl font-black text-[#C6A15B]">{dayReport.kpis.completionPct}%</div>
                        <div className="text-[10px] text-white/50 uppercase">Completion</div>
                      </div>
                    </div>
                    <div className="mt-3 h-2 bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full bg-[#C6A15B] rounded-full" style={{ width: `${dayReport.kpis.completionPct}%` }} />
                    </div>
                    {/* KPI row */}
                    ...
                    <div className="space-y-2">
                      {dayReport.moduleStats.map((m: any, i: number) => (
                        <div key={i} className="flex items-center gap-3">
                          <div className="text-xs font-bold text-[#4A1726] w-40 truncate flex-shrink-0">{m.module_name}</div>
                          ...
                          <div className="text-xs font-black text-[#4A1726] w-14 text-right flex-shrink-0">{m.done}/{m.total}</div>
                        ...
                        {dayReport.attentionItems.map((item: any, i: number) => (
                          <div key={i} className="px-4 py-2.5 flex items-start gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="text-xs font-bold text-[#4A1726]">{item.checkpoint_title}</div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <button onClick={() => navigate(`/mcheck-reports?date=${selectedDate}`)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-[#4A1726] text-white text-xs font-bold hover:bg-[#350E1A] transition-all shadow-sm">
                      <BarChart3 className="w-3.5 h-3.5 text-[#C6A15B]" /> Open Dashboard
                    </button>
