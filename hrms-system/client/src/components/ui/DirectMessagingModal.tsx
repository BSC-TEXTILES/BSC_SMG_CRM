...
export default function DirectMessagingModal({ isOpen, onClose, session }: DirectMessagingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
      <div className="w-full max-w-lg bg-white rounded-3xl overflow-hidden shadow-2xl border border-[#EAE4DC] flex flex-col h-[520px] animate-fade-in">
        {/* Header */}
        <div className="p-4 border-b border-[#EAE4DC] bg-[#4A1726] text-white flex items-center justify-between">
        ...
        {/* Recipient Selector */}
        <div className="p-3 bg-[#F8F5F1] border-b border-[#EAE4DC] flex items-center gap-2 text-xs">
          <span className="font-extrabold text-[#4A1726]">Send To:</span>
          <select
            value={recipient}
            onChange={(e) => setRecipient(e.target.value)}
            className="flex-1 p-1.5 rounded-xl border border-[#EAE4DC] bg-white font-bold text-[#4A1726]"
          >
        ...
        {/* Thread History */}
        <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#F8F5F1]/50 text-xs">
          {messages.length > 0 ? (
            messages.map((m) => {
                ...
                <div key={m.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  <div
                    className={`
                      ...
                      ${isMe ? 'bg-[#4A1726] text-white rounded-tr-none' : 'bg-white border border-[#EAE4DC] text-[#4A1726] rounded-tl-none'}
                    `}
                  >
                    {m.text}
                  </div>
        ...
        {/* Messaging Input */}
        <form onSubmit={handleSend} className="p-3 border-t border-[#EAE4DC] bg-white flex items-center gap-2">
          <input
            type="text"
            value={text}
