...
export default function ShortlistModal({ candidate, isOpen, onClose, onShortlistConfirmed }: ShortlistModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-[#4A1726]/70 backdrop-blur-md transition-all animate-fade-in select-none">
      <div className="relative w-full max-w-3xl max-h-[92vh] bg-[#F8F5F1] rounded-3xl shadow-2xl flex flex-col z-10 overflow-hidden border-2 border-[#C6A15B]/50">
            ...
            <div className="shrink-0">
              {photo ? (
                <img
                  src={photo}
                  alt={candidate.name}
                  className="w-16 h-16 rounded-2xl object-cover border-2 border-[#C6A15B] shadow-md bg-white p-0.5"
                  onError={(e) => { e.currentTarget.style.display = 'none'; }}
                />
              ) : (
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#4A1726] to-[#5C1D30] text-white font-black text-xl flex items-center justify-center border-2 border-[#C6A15B] shadow-md">
        ...
        <form onSubmit={handleConfirmShortlist} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5 text-xs">
          ...
          <div className="p-5 rounded-2xl bg-white border border-[#EAE4DC] shadow-xs space-y-4">
            <h3 className="font-black text-[#4A1726] uppercase text-xs tracking-wider border-b border-[#EAE4DC] pb-2.5 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div>
                <label className="block text-[11px] font-black text-[#4A1726] uppercase mb-1">
                  Retail Industry Experience
                </label>
                <select
                  value={retailExp}
                  onChange={(e) => setRetailExp(e.target.value)}
                  className="w-full px-3 py-2 rounded-xl border border-[#EAE4DC] text-xs font-bold text-[#4A1726] outline-none focus:ring-2 focus:ring-[#C6A15B]/40 bg-[#F8F5F1]"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-[#EAE4DC] text-xs font-mono font-bold text-[#4A1726] outline-none focus:ring-2 focus:ring-[#C6A15B]/40 bg-[#F8F5F1]"
                />
              </div>
          ...
          {/* Modal Sticky Footer */}
          <div className="p-4 bg-[#F8F5F1] rounded-2xl border border-[#EAE4DC] flex items-center justify-between">
            <div className="text-[11px] text-[#777777] font-bold">
              Candidate status updates to <span className="text-[#4A1726] font-black">Shortlisted</span>
            </div>
