      ...
      <div className="flex-1 lg:pl-64 flex flex-col min-w-0">
        ...
        <main className="p-4 lg:p-6 space-y-6 flex-1 overflow-y-auto">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-black text-[#4A1726] flex items-center gap-2">
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-[#4A1726] text-white text-xs font-bold hover:bg-[#350E1A] flex items-center gap-2 shadow-md transition-all"
            >
          ...
          <div className="card-glass p-4 overflow-x-auto">
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="border-b border-[#EAE4DC] text-xs font-black uppercase text-[#888888] tracking-wider">
              ...
              <tbody className="divide-y divide-[#EAE4DC]/50">
                {openings.map((op, idx) => {
                  ...
                  return (
                    <tr key={op.designation} className="hover:bg-black/5 transition-colors font-medium">
                      ...
                      <td className="py-4 px-4 text-[#4A1726] font-bold">{op.designation}</td>
                      <td className="py-4 px-4">
                        {isEditing ? (
                          <input 
                            ...
                            className="w-24 p-1.5 border border-[#4A1726] rounded-md font-bold text-[#4A1726] text-center bg-white"
                          />
                        ) : (
                          <span className="text-lg font-black text-[#4A1726]">{op.required}</span>
                        )}
                      </td>
                      <td className="py-4 px-4">
                        ...
                        {isEditing ? (
                          <div className="flex items-center justify-end gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg bg-[#4A1726] text-white font-bold flex items-center gap-1 hover:bg-[#350E1A] text-xs"
                            >
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg border border-[#EAE4DC] text-[#666666] font-bold hover:bg-white text-xs"
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg border border-[#4A1726] text-[#4A1726] font-bold text-xs hover:bg-[#4A1726] hover:text-white transition-colors flex items-center gap-1"
                            >
                ...
                {openings.length > 0 && (() => {
                  ...
                  return (
                    <tr className="bg-[#4A1726]/10 border-t-2 border-[#4A1726] font-black text-sm text-[#4A1726]">
                      <td className="py-4 px-4 font-black uppercase text-xs tracking-wider">
                        Total Manpower Summary
                      </td>
                      <td className="py-4 px-4">
                        <span className="text-xl font-black text-[#4A1726]">{totalRequired}</span>
                        <div className="text-[10px] text-[#4A1726]/70 font-bold uppercase mt-0.5">Total Openings</div>
                      </td>
                      <td className="py-4 px-4">
                      ...
                      <td className="py-4 px-4 text-right">
                        <span className="px-2.5 py-1 rounded-full bg-[#4A1726] text-white font-bold text-[10px] uppercase">Overall Total</span>
                      </td>
                    </tr>
                  );
                })()}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white rounded-2xl p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#EAE4DC] pb-3">
              <h3 className="font-extrabold text-[#4A1726] text-base">Add New Role / Designation</h3>
              <button onClick={() => setAddModalOpen(false)} className="text-[#888888] hover:text-[#4A1726]">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 rounded-lg border border-[#EAE4DC] bg-[#F8F5F1] font-bold text-[#4A1726]"
                />
              </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-[#EAE4DC]">
              <button
                ...
                className="px-4 py-2 rounded-lg border border-[#EAE4DC] text-xs font-bold"
              >
                Cancel
              </button>
              <button
                ...
                disabled={addingRole}
                className="px-4 py-2 rounded-lg bg-[#4A1726] text-white text-xs font-bold disabled:opacity-50"
              >
