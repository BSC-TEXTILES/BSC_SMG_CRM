...
export default function InterviewPanelPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F5F1] flex">
      ...
      <div className="flex-1 lg:pl-64 flex flex-col min-w-0">
        ...
        <main className="p-4 lg:p-6 space-y-6 flex-1 overflow-y-auto">
          ...
          <div className="card-glass p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-black text-[#4A1726] tracking-tight flex items-center gap-2">
                <Target className="w-5 h-5 text-[#C6A15B]" />
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-3 py-1.5 rounded-xl border border-[#EAE4DC] bg-[#F8F5F1] text-xs font-semibold text-[#4A1726] focus:outline-none focus:border-[#4A1726] w-56 shadow-xs"
                />
              </div>
            </div>
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`
                  ...
                  ${activeFilter === f.key 
                    ? 'bg-[#4A1726] text-white border-[#4A1726] font-black' 
                    : 'bg-white text-[#555555] border-[#EAE4DC] hover:bg-[#F8F5F1]'}
                `}
              >
                {f.label}
              </button>
                ...
                <tbody className="divide-y divide-[#EAE4DC]/60">
                  {filtered.map((iv, idx) => (
                    <tr key={iv.appNo} className="hover:bg-black/5 transition-colors font-medium">
                      ...
                      <td className="py-3.5 px-4 font-extrabold text-[#4A1726]">{iv.candidate}</td>
                      ...
                      <td className="py-3.5 px-4">
                        {iv.assignedName ? (
                          <div className="font-bold text-[#4A1726]">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg border border-[#4A1726] text-[#4A1726] font-bold text-[11px] hover:bg-[#4A1726] hover:text-white transition-all flex items-center gap-1 shadow-xs"
                          >
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          {session?.role !== 'Manager' && (
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-[#4A1726] text-white font-bold text-[11px] hover:bg-[#350E1A] shadow-xs"
                            >
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg border border-[#4A1726] text-[#4A1726] font-bold text-[11px] hover:bg-[#4A1726] hover:text-white shadow-xs transition-colors"
                          >
                            View Scorecard
                          </button>
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-[#C6A15B] text-white font-bold text-[11px] hover:bg-amber-600 shadow-xs"
                          >
      ...
      {scorePanel.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-[#4A1726]/60 backdrop-blur-md animate-fade-in">
          <div className="relative w-full max-w-3xl bg-white rounded-3xl shadow-2xl border border-[#EAE4DC] overflow-hidden flex flex-col max-h-[90vh] my-auto animate-scale-in">
            {/* Modal Header */}
            <div className="p-4 sm:p-6 border-b border-[#EAE4DC] bg-[#4A1726] text-white flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-[#C6A15B]/20 border border-[#C6A15B]/40 flex items-center justify-center text-[#C6A15B]">
                  <Award className="w-5 h-5" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-[#C6A15B] text-white">
            ...
            <div className="p-4 sm:p-6 space-y-5 overflow-y-auto flex-1 text-xs">
              {scorePanel.round === 'Round 2' && scorePanel.interview?.hrScore && scorePanel.hrQuestions && (
                <div className="mb-6 bg-blue-50/50 border border-blue-100 rounded-2xl p-4">
                  <h4 className="font-extrabold text-[#4A1726] text-xs uppercase tracking-wider mb-3">HR Round 1 Evaluation Summary</h4>
                  <div className="space-y-2 mb-3">
                    {scorePanel.hrQuestions.map((hq: any, idx: number) => (
                      <div key={hq.id} className="flex justify-between items-center bg-white p-2.5 rounded-xl border border-[#EAE4DC]">
                        <span className="font-bold text-[#4A1726] text-[11px] max-w-[80%]">{hq.text}</span>
                        <span className="font-black text-xs text-[#C6A15B]">
              ...
              <div className="space-y-3">
                <h4 className="font-extrabold text-[#4A1726] text-xs uppercase tracking-wider flex items-center gap-2">
                ...
                {scorePanel.questions.map((q, idx) => (
                  <div key={idx} className="p-4 rounded-2xl border border-[#EAE4DC] bg-[#F8F5F1] space-y-3">
                    <div className="flex items-start justify-between gap-3 font-bold text-[#4A1726]">
                      <span className="text-xs sm:text-sm font-extrabold leading-snug">{idx + 1}. {q.question}</span>
                      <span className="px-2.5 py-1 rounded-lg bg-[#4A1726]/10 text-[#4A1726] font-mono text-[11px] font-black flex-shrink-0">
                        Max: {q.max || 10}
                      </span>
                    </div>
                    <div className="flex items-center gap-4 bg-white p-2.5 rounded-xl border border-[#EAE4DC]">
                      <input
                        type="range"
                        min="0"
                        max={q.max || 10}
                        ...
                        className="w-full accent-[#4A1726] cursor-pointer"
                      />
                      <span className="font-black text-base text-[#4A1726] w-8 text-center bg-[#F8F5F1] py-1 rounded-lg border border-[#EAE4DC]">
                        {scores[idx] || 0}
                      </span>
                    </div>
                  </div>
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-r from-[#4A1726] to-[#350E1A] text-white flex items-center justify-between shadow-lg border border-[#C6A15B]/30">
                <div>
                ...
                <div className="text-right">
                  <span className="text-2xl font-black text-[#C6A15B]">
              ...
              <div className="space-y-4 pt-2">
                <div>
                  <label className="block font-extrabold text-[#4A1726] mb-1 text-xs">Evaluator Remarks &amp; Observations *</label>
                  <textarea
                    rows={3}
                    value={remarks}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-extrabold text-[#4A1726] mb-1 text-xs">
            ...
            {/* Modal Footer */}
            <div className="p-4 sm:p-5 border-t border-[#EAE4DC] bg-[#F8F5F1] flex items-center justify-end gap-3 sticky bottom-0 z-10">
              <button
                onClick={() => setScorePanel({ open: false, interview: null, questions: [], round: 'HR' })}
                className="px-5 py-2.5 rounded-xl border-2 border-[#EAE4DC] bg-white text-[#555555] font-extrabold hover:bg-black/5 transition-colors text-xs"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveScore}
                className="px-6 py-2.5 rounded-xl bg-[#4A1726] text-white font-black hover:bg-[#350E1A] shadow-md transition-all text-xs flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
          <div className="w-full max-w-md bg-white rounded-2xl p-6 space-y-4 shadow-2xl animate-fade-in">
            <div className="flex items-center justify-between border-b border-[#EAE4DC] pb-3">
              <h3 className="font-extrabold text-[#4A1726] text-base">Assign Round 2 Evaluator</h3>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-[#4A1726] mb-1">Evaluator Full Name *</label>
                <input type="text" value={evalName} onChange={(e) => setEvalName(e.target.value)} placeholder="e.g. Rajesh Kumar" className="input-modern" />
              </div>
              <div>
                <label className="block font-bold text-[#4A1726] mb-1">Evaluator Designation *</label>
              ...
              {generatedLink && (
                <div className="p-3 rounded-xl bg-[#F8F5F1] border border-[#EAE4DC] space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 rounded bg-[#4A1726] text-white font-bold"
                    >
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-[#EAE4DC]">
              <button onClick={() => setAssignModal({ open: false, interview: null })} className="px-4 py-2 rounded-xl border border-[#EAE4DC] font-bold text-xs">
                Close
              </button>
              {!generatedLink && (
      ...
      {approveModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
          <div className="w-full max-w-md bg-white rounded-2xl p-6 space-y-4 shadow-2xl animate-fade-in">
            <div className="flex items-center justify-between border-b border-[#EAE4DC] pb-3">
              <h3 className="font-extrabold text-[#4A1726] text-base">Approve Candidate Selection</h3>
            ...
            <div className="space-y-3 text-xs max-h-[60vh] overflow-y-auto pr-1">
              ...
              {approveModal.interview?.hrScore && (
                <div className="bg-[#F8F5F1] p-3 rounded-xl border border-[#EAE4DC]">
                  <h4 className="font-extrabold text-[#4A1726] text-[10px] uppercase tracking-wider mb-1">HR Round 1 Summary</h4>
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-bold text-[#555555]">Score:</span>
                    <span className="font-black text-[#C6A15B]">{approveModal.interview.hrScore.total} / {approveModal.interview.hrScore.maxTotal}</span>
                  </div>
              ...
              {approveModal.interview?.assignedScore && (
                <div className="bg-blue-50 p-3 rounded-xl border border-blue-100">
                  <h4 className="font-extrabold text-[#4A1726] text-[10px] uppercase tracking-wider mb-1">Round 2 Management Summary</h4>
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-bold text-[#555555]">Score:</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                <div>
                  <label className="block font-bold text-[#4A1726] mb-1">Salary Offered (₹) *</label>
                ...
                <div>
                  <label className="block font-bold text-[#4A1726] mb-1">Expected DOJ *</label>
                  <input type="date" value={approveDoj} onChange={(e) => setApproveDoj(e.target.value)} className="input-modern" />
                </div>
                <div>
                  <label className="block font-bold text-[#4A1726] mb-1">Finalized Designation *</label>
                ...
                <div>
                  <label className="block font-bold text-[#4A1726] mb-1">Allocated Department *</label>
                ...
                <div className="sm:col-span-2">
                  <label className="block font-bold text-[#4A1726] mb-1">Notice Period (Optional)</label>
              ...
              <div>
                <label className="block font-bold text-[#4A1726] mb-1">Approval Remarks</label>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-[#EAE4DC]">
              <button onClick={() => setApproveModal({ open: false, interview: null, probation: false })} className="px-4 py-2 rounded-xl border border-[#EAE4DC] font-bold text-xs">
                Cancel
              </button>
      ...
      {viewScorecardModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-[#4A1726]/60 backdrop-blur-md">
          <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-in">
            <div className="p-4 sm:p-5 border-b border-[#EAE4DC] bg-[#4A1726] text-white flex items-center justify-between sticky top-0 z-10">
              <div>
            ...
            <div className="p-4 sm:p-6 space-y-6 overflow-y-auto flex-1 text-xs">
              {viewScorecardModal.interview?.hrScore ? (
                <div className="bg-[#F8F5F1] border border-[#EAE4DC] rounded-2xl p-4 sm:p-5">
                  <div className="flex justify-between items-center mb-4">
                    <h4 className="font-extrabold text-[#4A1726] text-sm uppercase tracking-wider">HR Round 1 Details</h4>
                    <span className="font-black text-lg text-[#C6A15B]">{viewScorecardModal.interview.hrScore.total} / {viewScorecardModal.interview.hrScore.maxTotal}</span>
                  </div>
                  <div className="space-y-2 mb-4">
                    {viewScorecardModal.hrQuestions.map((hq: any, idx: number) => (
                      <div key={hq.id} className="flex justify-between items-center bg-white p-3 rounded-xl border border-[#EAE4DC]">
                        <span className="font-bold text-[#4A1726] text-[11px] max-w-[80%]">{hq.text}</span>
                        <span className="font-black text-xs text-[#C6A15B]">
                  ...
                  <div className="bg-white p-3 rounded-xl border border-[#EAE4DC]">
                    <span className="block text-[10px] font-black uppercase text-[#777777] mb-1">Evaluator Remarks</span>
                    <p className="text-xs font-medium text-[#4A1726]">
                      {viewScorecardModal.interview.hrScore.remarks || 'No remarks provided.'}
                    </p>
                  </div>
                </div>
              ) : (
                <div className="text-[#888888] font-medium text-center py-4 bg-[#F8F5F1] rounded-2xl border border-[#EAE4DC] border-dashed">
