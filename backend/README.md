...
- **Frontend:** Vite + React 18, TypeScript, Tailwind CSS, Lucide React icons, custom Burgundy & Champagne design system.
- **Backend:** Node.js, Express.js REST API (modern `/api/v1` + legacy `/api` dispatcher), `mysql2/promise` connection pool, JWT Authentication, Multer file uploader, bcrypt password hashing, Socket.IO real-time events.
- **Database:** MySQL 8.0 / MariaDB (Hostinger compatible) with self-healing auto-initializer.
- **Security:** Helmet headers, CORS, login rate limiting, role-based route guards (`HR`, `Manager`, `Admin`, `Super Admin`), AES-256-GCM field encryption, DevTools shield (see *Security Architecture* below).
...
- **Password storage:** all passwords are stored as **bcrypt hashes**. Any legacy plaintext row found in the
  database is transparently upgraded to a bcrypt hash the first time that user logs in — no data migration needed.
- **Master recovery credential:** the deployment recovery login is `admin@bsctextiles.com` / `admin@2026`
  (works even before the database is seeded). It unlocks only the four built-in deployment accounts — never
  arbitrary database accounts.
- **JWT secrets:** `JWT_SECRET` / `JWT_REFRESH_SECRET` come from environment variables. If they are not set, the
  server generates a cryptographically strong value on first boot and persists it to
  `server/config/.runtime-secrets.json` (gitignored). The old hardcoded fallback secret is gone.
- **Login rate limiting:** 50 attempts / 10 min / IP on all credential endpoints.
- **No secret leakage:** database and internal errors are logged server-side; clients only ever receive
  sanitized business messages.

### Encryption at Rest

- Free-text PII in the Wedding CRM (customer notes, call remarks) is encrypted with **AES-256-GCM**
  (`server/utils/crypto.js`) using the `ENCRYPTION_KEY` env var (auto-generated & persisted if omitted).
- Wire format: `enc:v1:<iv>:<authTag>:<ciphertext>`; decryption is dual-read — legacy plaintext rows keep
  working and are re-encrypted on their next write. Tampering with an encrypted value is detected via the
  GCM auth tag.

### Endpoint Authorization

- Destructive maintenance endpoints (`/api/wipe-db`, `/api/fix-db-schema`, `/api/public/migrate-db`)
  are **Admin-only**.
- The legacy action dispatcher (`POST /api/legacy`) requires a valid token for every non-public action and
  enforces Admin-only on user management / page visibility / interview-question administration.
- All Wedding CRM routes require authentication; customer deletion is Admin-only. Multi-location isolation
  is enforced server-side from the JWT, never from the client.

### Developer Tools Shield

- The client ships a **DevToolsGuard** (`client/src/components/DevToolsGuard.tsx`) that detects open browser
  developer tools on **all devices** (desktop + mobile) using three independent signals: viewport delta,
  debugger pause timing, and a DOM getter probe.
- When detected, the app locks behind a full-screen **"Please turn off Developer Tools"** overlay and unlocks
  automatically once the tools are closed.
- Every detection is reported to `POST /api/security/log-event` and is visible to admins under
  **System Settings → Security & DevTools Shield**.
- The shield is bypassed on `localhost` / `127.0.0.1` so developers can work normally.
