...
export default function DepartmentHiringPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F5F1] flex">
      ...
      <div className="flex-1 lg:pl-64 flex flex-col min-w-0">
        ...
        <main className="p-4 lg:p-6 space-y-6 flex-1 overflow-y-auto">
          ...
          <div className="card-glass p-4 space-y-3">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-[#EAE4DC] pb-2.5">
              <div className="text-xs font-black text-[#4A1726] uppercase tracking-wider flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-2.5 py-1 rounded-xl bg-white border border-[#EAE4DC] text-[#4A1726] hover:bg-[#F8F5F1] text-xs font-extrabold flex items-center gap-1 shadow-xs"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-2.5 py-1.5 rounded-xl border border-[#EAE4DC] bg-[#F8F5F1] text-xs font-bold text-[#4A1726] focus:outline-none focus:border-[#4A1726]"
                >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-2.5 py-1.5 rounded-xl border border-[#EAE4DC] bg-[#F8F5F1] text-xs font-bold text-[#4A1726] focus:outline-none focus:border-[#4A1726]"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="card-glass p-5 space-y-4">
            <div className="flex items-center justify-between border-b border-[#EAE4DC] pb-3">
              <div>
                <h3 className="font-extrabold text-[#4A1726] text-base tracking-tight flex items-center gap-2">
                  <FolderOpen className="w-5 h-5 text-[#C6A15B]" />
                  <span>Showroom Department &amp; Section Workforce Tree</span>
                </h3>
                <p className="text-xs text-[#777777] font-medium mt-0.5">
                  Designation Target: <span className="font-bold text-[#4A1726] bg-[#C6A15B]/20 px-2 py-0.5 rounded-md">Sales Executive</span>
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {filteredTreeHierarchy.length > 0 ? (
                filteredTreeHierarchy.map((deptNode) => {
                  ...
                  return (
                    <div 
                      ...
                      className="border border-[#EAE4DC] rounded-2xl bg-white/80 overflow-hidden shadow-xs transition-all"
                    >
                      {/* Department Root Row */}
                      <div 
                        onClick={() => toggleExpandDept(deptNode.department)}
                        className="p-4 bg-[#F8F5F1] hover:bg-[#F8F5F1]/60 cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#EAE4DC]/80"
                      >
                        <div className="flex items-center gap-3">
                          <button className="p-1 rounded-lg bg-[#4A1726]/10 text-[#4A1726] hover:bg-[#4A1726] hover:text-white transition-colors">
                            {isExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                          </button>
                          <Building2 className="w-5 h-5 text-[#4A1726]" />
                          <div>
                            <h4 className="font-black text-[#4A1726] text-base tracking-tight">
                              {deptNode.department}
                            </h4>
                        ...
                        <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                          <div className="text-right text-xs">
                            ...
                            <span className="font-extrabold text-[#4A1726] text-sm">{deptNode.required}</span>
                          </div>
                          ...
                          <div className="w-36 sm:w-44 space-y-1">
                            <div className="flex items-center justify-between text-[11px] font-extrabold text-[#4A1726]">
                              <span>{deptNode.filled} / {deptNode.required}</span>
                              <span>{deptNode.percentage}%</span>
                            </div>
                            <div className="w-full h-2.5 bg-[#EAE4DC] rounded-full overflow-hidden shadow-inner">
                              <div 
                                className={`h-full rounded-full transition-all duration-500 ${
                                  deptNode.percentage >= 100 ? 'bg-emerald-600' :
                                  deptNode.percentage >= 75 ? 'bg-[#C6A15B]' :
                      ...
                      {isExpanded && (
                        <div className="divide-y divide-[#EAE4DC]/50 bg-white">
                          {deptNode.sections.map((secNode) => {
                            ...
                            return (
                              <div 
                                key={secNode.section}
                                className="p-3.5 pl-6 sm:pl-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-[#F8F5F1]/80 transition-colors font-medium border-l-4 border-l-[#C6A15B]/40 ml-3 sm:ml-6 my-1"
                              >
                                <div className="flex items-center gap-3">
                                  <span className="text-[#C6A15B] font-mono font-bold text-sm">├──</span>
                                  <Layers className="w-4 h-4 text-[#C6A15B]" />
                                  <div>
                                    <span className="font-extrabold text-[#4A1726] text-xs sm:text-sm">
                                      {secNode.section}
                                    </span>
                                ...
                                <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                                  <div className="text-right text-xs">
                                    ...
                                    <span className="font-extrabold text-[#4A1726]">{secNode.required}</span>
                                  </div>
                                  ...
                                  <div className="w-32 sm:w-36 space-y-1">
                                    <div className="flex items-center justify-between text-[10.5px] font-bold text-[#4A1726]">
                                    ...
                                    <div className="w-full h-2 bg-[#EAE4DC] rounded-full overflow-hidden">
                                      <div 
                                        className={`h-full rounded-full transition-all duration-500 ${
                                          ...
                                          secNode.percentage >= 75 ? 'bg-[#C6A15B]' :
                                  ...
                                  {isHR && (
                                    <button
                                      ...
                                      className="px-2.5 py-1 rounded-lg bg-[#4A1726]/10 text-[#4A1726] hover:bg-[#4A1726] hover:text-white font-bold text-xs transition-colors flex items-center gap-1"
                                    >
                ...
                <div className="py-12 text-center text-xs text-[#777777] font-semibold bg-white rounded-2xl border border-[#EAE4DC]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="card-glass p-5 space-y-4">
              <h3 className="font-extrabold text-[#4A1726] text-sm tracking-tight flex items-center gap-2">
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={deptChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="department" tick={{ fontSize: 10, fontWeight: 700, fill: '#4A1726' }} />
                    <YAxis tick={{ fontSize: 10, fontWeight: 700, fill: '#4A1726' }} />
                    <Tooltip contentStyle={{ backgroundColor: '#4A1726', borderRadius: '12px', color: '#fff', fontSize: '12px' }} />
                    <Legend wrapperStyle={{ fontSize: '11px', fontWeight: 700 }} />
                    <Bar dataKey="required" name="Required Target" fill="#4A1726" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="filled" name="Filled Staff" fill="#C6A15B" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="card-glass p-5 space-y-4 flex flex-col justify-between">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Top Performing */}
                <div className="p-3.5 rounded-2xl bg-white border border-[#EAE4DC] space-y-2.5">
                  ...
                  <div className="space-y-2 text-xs">
                    {topPerformingDepts.map(d => (
                      <div key={d.department} className="flex justify-between items-center font-semibold">
                        <span className="text-[#4A1726]">{d.department}</span>
      ...
      {editModal.open && (
        <div className="fixed inset-0 z-50 bg-[#4A1726]/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#F8F5F1] rounded-2xl border border-[#EAE4DC] shadow-2xl w-full max-w-md p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-[#EAE4DC] pb-3">
              <div>
                <h3 className="font-extrabold text-[#4A1726] text-base">Edit Sales Executive Target</h3>
              ...
              <button 
                ...
                className="w-8 h-8 rounded-full bg-[#4A1726]/10 hover:bg-[#4A1726] hover:text-white text-[#4A1726] flex items-center justify-center transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="space-y-4 text-xs font-semibold text-[#4A1726]">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-[#EAE4DC] bg-white font-bold text-sm"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-[#EAE4DC] bg-white font-medium text-xs focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-[#EAE4DC]">
              <button
                onClick={() => setEditModal(prev => ({ ...prev, open: false }))}
                className="px-4 py-2 rounded-xl border border-[#EAE4DC] bg-white text-[#4A1726] font-bold text-xs hover:bg-[#F8F5F1]"
              >
                Cancel
              </button>
              <button
