...
export default function Sidebar({ session, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-[#163B5C]/60 backdrop-blur-xs z-40 lg:hidden transition-opacity"
          onClick={onClose}
        />
      )}
      ...
      <aside
        ...
        className={`
          fixed top-0 left-0 bottom-0 bg-[#163B5C] text-white z-50 flex flex-col transition-all duration-300 shadow-2xl border-r border-[#4E8ABF]/25
      ...
      >
        ...
        {collapsed ? (
          <div className="p-3 border-b border-[#4E8ABF]/15 flex flex-col items-center justify-center min-h-[64px] gap-2.5">
            {/* 3-line hamburger button prominently displayed at top */}
            <button
              type="button"
              onClick={handleToggle}
              className="p-1.5 rounded-xl text-[#4E8ABF] hover:text-white hover:bg-white/10 transition-colors flex items-center justify-center cursor-pointer shadow-xs border border-[#4E8ABF]/30"
              title="Expand navigation menu (3 lines)"
              aria-label="Expand sidebar"
            >
              <Menu className="w-5 h-5 text-[#4E8ABF]" />
            </button>
            {/* ONLY LOGO */}
            <img 
              src="/logo.png" 
              alt="BSC Logo" 
              className="w-9 h-9 object-contain rounded-xl bg-white p-1 shadow-md border border-[#4E8ABF]/40 hover:scale-105 transition-transform cursor-pointer"
              onClick={handleToggle}
              title="BSC Logo - Click to expand navigation"
            />
          </div>
        ) : (
          <div className="p-3.5 border-b border-[#4E8ABF]/15 flex items-center justify-between min-h-[64px] w-full">
            <div className="flex items-center gap-2.5 min-w-0">
              <img 
                src="/logo.png" 
                alt="BSC Logo" 
                className="w-10 h-10 object-contain rounded-xl bg-white p-1 shadow-md border border-[#4E8ABF]/30 flex-shrink-0" 
              />
              <div className="min-w-0">
                <div className="font-extrabold text-sm text-[#F4F6F9] tracking-wide leading-tight truncate">BSC EXCLUSIVE</div>
                <div className="text-[9px] font-bold uppercase tracking-widest mt-0.5 flex items-center gap-1 truncate text-[#4E8ABF]">
            ...
            <button
              type="button"
              onClick={handleToggle}
              className="p-1.5 rounded-xl text-[#4E8ABF] hover:text-white hover:bg-white/10 transition-colors flex-shrink-0 cursor-pointer border border-[#4E8ABF]/30 shadow-xs"
        ...
        {/* User Card */}
        <div className={`mx-2 my-2 rounded-xl bg-black/20 border border-[#4E8ABF]/25 flex items-center shadow-inner transition-all ${
          collapsed ? 'p-1 justify-center' : 'p-2.5 gap-2.5'
        }`}>
          <div 
            className="w-8 h-8 rounded-lg bg-[#4E8ABF] text-[#1B2A3B] font-black flex items-center justify-center text-xs shadow-md border border-[#6FA3D0] flex-shrink-0"
          ...
          {!collapsed && (
            <div className="overflow-hidden flex-1">
              <div className="font-bold text-xs text-[#F4F6F9] truncate">{session?.fullName || 'HR Manager'}</div>
              <div className="text-[10px] text-[#4E8ABF] font-semibold truncate">{roleLabels[role] || role}</div>
            </div>
          )}
        </div>
          ...
          {['Store Operations', 'Core Workspace', 'Daily Operations', 'Talent Management', 'Public Portals', 'Administration'].map(section => {
            ...
            return (
              <div key={section} className="space-y-0.5">
                {collapsed ? (
                  <div className="h-px bg-[#4E8ABF]/20 my-1.5 mx-1" />
                ) : (
                  <div className="text-[9px] font-black uppercase tracking-widest text-[#4E8ABF]/60 px-2.5 mb-1">
                    <span>{section}</span>
                  </div>
                )}
                    ...
                    return (
                      <Link
                        ...
                        className={`
                          ...
                          ${isActive 
                            ? 'bg-[#4E8ABF] text-[#1B2A3B] shadow-lg shadow-[#4E8ABF]/25 font-black border-l-4 border-[#1B2A3B]' 
                            : 'text-[#F4F6F9]/85 hover:bg-[#1F4D77] hover:text-white'}
                        `}
                      >
                        <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-2.5 min-w-0'}`}>
                          <Icon className={`w-4 h-4 transition-transform group-hover:scale-110 flex-shrink-0 ${
                            isActive ? 'text-[#1B2A3B]' : item.key === 'wedding_crm' ? 'text-[#6FA3D0] animate-pulse' : 'text-[#4E8ABF]/80 group-hover:text-[#4E8ABF]'
                          }`} />
                          ...
                          {!collapsed && item.key === 'wedding_crm' && (
                            <span className="text-[8px] bg-[#4E8ABF] text-[#1B2A3B] font-black px-1.5 py-[2px] rounded-full uppercase ml-1 flex-shrink-0">
                              NEW
                            </span>
                          )}
                        </div>
                        ...
                        {!collapsed && isActive && (
                          <ChevronRight className="w-3.5 h-3.5 text-[#1B2A3B] opacity-80 flex-shrink-0" />
                        )}
                      </Link>
                    );
                  })}
        ...
        {/* Footer Logout */}
        <div className={`border-t border-[#4E8ABF]/15 bg-[#0E2A44]/80 transition-all ${collapsed ? 'p-2' : 'p-3'}`}>
          <button
            onClick={() => Auth.logout()}
            title="Sign Out Session"
            className={`w-full flex items-center justify-center rounded-xl text-xs font-bold bg-[#C43D4B]/15 text-[#F4F6F9] border border-[#C43D4B]/40 hover:bg-[#C43D4B] hover:text-white transition-all shadow-sm ${
          ...
          {!collapsed && (
            <div className="text-[8.5px] text-[#F4F6F9]/40 text-center mt-2 font-medium">
